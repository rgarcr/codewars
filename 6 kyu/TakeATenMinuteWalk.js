...
function isValidWalk(walk) {
    ...
    for (let letter of walk)
        countObj[letter] > 0 ? countObj[letter]++ : countObj[letter] = 1;

    let countArray = Object.values(countObj)
    let maxDirection = Math.max(...countArray);
    if (maxDirection > 5 || Object.keys(countObj).length % 2 !== 0) return false;

    for (let key in countObj) {
        if (key === 'n' || key === 's')
            if (countObj['n'] !== countObj['s'])
                return false;
        if (key === 'e' || key === 'w')
            if (countObj['e'] !== countObj['w'])
                return false;
    }
    return true;
}